...
var Promise = require('simple-promise'),
    _ = require('noeca-lib').Underscore;
 ...
 *  - rnd (Sring): (optional) Store uniqe id used to mark environment as being
 *    preloaded etc. Generated randomly by default upon store initialization
 *    (construction).
 *
 *
...
Store.prototype.preload = function preload(env, callback) {
  ...
  if (shared.stores[this.rnd]) {
    shared.stores[this.rnd].done(callback);
    return;
  }

  shared.stores[this.rnd] = new Promise(callback);
  this.__.preloader.call(this, env, shared.stores[this.rnd].resolve);
};
