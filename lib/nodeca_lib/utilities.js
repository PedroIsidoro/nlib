...
/** alias: Utilities.underscore
 *  Utilities._ -> Underscore
 **/
/**
 *  Utilities.underscore -> Underscore
 *  
 *  Exported utility-belt.
 *  See: http://documentcloud.github.com/underscore/
 **/
Utilities.underscore = Utilities._ = require('underscore')
...
var select_backends = {
  array: function select_array(arr, callback) {
    var result = [];

    Utilities.each(arr, function (k, v) {
      if (callback(v, k)) {
        result.push(v);
      }
    });
  ...
  object: function select_object(obj, callback) {
    var result = {};

    Utilities.each(obj, function (k, v) {
      if (callback(v, k)) {
        result[k] = v;
      }
    });
...
Utilities.select = function select(data, callback) {
  ...
  var func = select_backends[Array.isArray(data) ? 'array' : 'object'];
  return func(data, callback);
};


Utilities.reject = function reject(data, callback) {
  if ('function' === typeof callback) {
    return Utilities.select(data, function (v, k) {
      return !callback(v, k);
    });
  }
...
Utilities.reverse = function reverse(arr) {
  var ret = [], idx = arr.length;
  while (idx--) { ret.push(arr[idx]); }
  return ret;
};



Utilities.inject = function inject(arr, memo, callback) {
...
Utilities.includes = function includes(arr, val) {
  var result = false;
...
var map_backends = {
  array: function map_array(arr, callback) {
    var result = [];
  ...
  object: function map_object(obj, callback) {
    var result = {};
...
Utilities.map = function map(arr, callback) {
  var func = map_backends[Array.isArray(data) ? 'array' : 'object'];
  return func(data, callback);
};
