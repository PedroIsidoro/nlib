...
var load_models = function load_models(next) {
  ...
  _.each(context.shared.schemas, function (schema, name) {
    ...
    var nice_name = _.camelize(name).capitalize(),
    ...
    app.config.routes.forEach(function (params) {
      ...
      try {  
        ...
        if (params.root) {
          ...
          app.router.match(_.delete(params, 'match'), params);
        } else if (params.resource) {
          app.router.resource(_.delete(params, 'resource'), params);
        } else {
...
var cleanup = function cleanup(next) {
  var context = this,
      views_builder = _.delete(context.shared, 'viewsBuilder'),
      static_builder = _.delete(context.shared, 'staticBuilder');
