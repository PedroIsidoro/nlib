...
var load_subapps = function load_subapps(next) {
  ...
  $$.each(context.main.config.modules, function (module, appConfig) {
...
var load_app_stores = function load_app_stores(app, callback) {
  var context = this,
      dirname = path.join(app.dirname, 'config', 'stores'),
      suffix_re = new RegExp('\.js$');
  ...
  path.exists(dirname, function (exists) {
    ...
    fs.readdir(dirname, function (err, files) {
      ...
      finished = new Promise(callback); // ALL stores of app loaded
      ...
      files.forEach(function (file) {
        ...
        try {
          name = $$.parameterize(file.replace(suffix_re, '')),
          app.settings.store(name, require(path.join(dirname, file)));
        } catch (err) {
          // stop with an error
          finished.resolve(err);
          return;
        }
      });
    ...
    fs.readdir(dirname, function (err, files) {
      ...
      files.forEach(function (file) {
        ...
        Yaml.readFile(path.join(dirname, file), function (err, data) {
          ...
          $$.each(data, function (store, settings) {
            $$.each(settings, function (key, definition) {
...
var cleanup = function cleanup(next) {
  var context = this,
      views_builder = $$.grab(context.shared, 'viewsBuilder'),
      static_builder = $$.grab(context.shared, 'staticBuilder');
