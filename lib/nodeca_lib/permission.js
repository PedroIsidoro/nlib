...
// runs all given tests within joint promise and fires callback when done
// each test promise resolved as `(err, result)`, where result is either
// `{success: true}` or `{failure: onFail}`
var run_tests = function run_tests(arr, env, callback) {
  ...
  arr.forEach(function (test) {
    ...
    test.handler.call(env, function (err, result) {
      ...
      promise.resolve(null, {failure: test.onFail});
    });
  });
 ...
 *
 *  ** WARNING **
 *  This constructor is called from nodeca-lib.Controller, and getter is ALWAYS
 *  got assigned to [[Application#settings]]'s [[Settings#get]] method.
 *
 *
 *  ##### Handling Failures
 *
...
Permission.prototype.filter = function permission_filter(req, res, next) {
  var self = this;
  // run OR tests
  run_tests(self.__.tests.OR, req, function (err) {
    var result = {failure: self.__.onFail};
    ...
    $$.each(Array.prototype.slice(arguments, 1), function (pArgs) {
      if (pArgs[1].success) {
        result = pArgs[1];
        return false;
      }
    ...
    if (result.success) {
      run_tests(self.__.tests.AND, req, function (err) {
        if (err) {
          next(err);
          return;
        }
        ...
        handle_failure(merge_on_fail(self.__.onFail, result.failure), req, res, next);
      });
      return;
    }

    handle_failure(result.failure, req, res, next);
  });
};
 ...
 *
 *
 *  ** WARNING ** `onFail` is ignored when `operation` is `OR`.
 *
 *
 ...
 *
 *  Syntax sugar. Shortcut for `test('OR', condition)`.
 *
 *  ##### See Also
 *
 *  - [[Permission#test]]
