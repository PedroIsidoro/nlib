...
var Application = module.exports = function Application(dirname, bootstrapper) {
  ...
  // once config is ready define new getter
  config_ready.done(function (err, config) {
    var getter  = !!err ? (function () { throw err; })
                        : (function () { return config; });

    delete self.config;
    self.__defineGetter__('config', getter);
  });
  ...
  this.bootstrap = function bootstrap(appConfig, callback) {
    ...
    config_ready.done(function (err, appDefaults) {
