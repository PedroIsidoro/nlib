...
// internal
var Test = require('./permission/test');
...
var merge_on_fail = function (generic, override) {
  var handler, result = {message: override.message || generic.message};
...
var run_tests = function run_tests(arr, env, callback) {
  Async.mapSeries(arr, function (test, next) {
    test.handler.call(env, function (err, result) {
      next(err, !!result ? {ok: true} : {fail: test.onFail});
    });
  }, callback);
};
      ...
      default:
        if ('(' !== token[0]) {
          ...
          while (token && 0 < depth) {
            token = parts.shift();
            if ('(' === token[0]) { depth += 1; }
            if (')' === token[token.length - 1]) { depth -= 1; }
            inner += token;
          }
...
Permission.prototype.filter = function permission_filter(req, res, next) {
  ...
  if (0 === this.__.tests.OR.length && 0 === this.__.tests.AND.length) {
    next(new Error("Permission has no tests defined."));
    return;
  }
  ...
  run_tests(self.__.tests.OR, function (err, results) {
    ...
    if (!_.any(results, function (r) { return r.ok; })) {
    ...
    run_tests(self.__.tests.AND, function (err, results) {
      ...
      var r = _.detect(results, function (r) { return !r.ok; });
...
Permission.prototype.addTest = function addTest(operation, condition, onFail) {
  ...
  condition = ('function' === typeof condition) ? new Test(condition)
