...
function concat_files(dst_file) {
  ...
  src_files = _.toArray(arguments).slice(1);
  callback  = src_files.pop();
  src_files = _.flatten(src_files);
...
function filter_object(obj, test) {
  var result = {};
 ...
 *  - init_env (Object): loaded apps, registerView callback
...
module.exports.bundle = function(nodeca, init_env, callback) {
  ...
  Async.waterfall([
    ...
    function move_base_themes(next) {
      ...
      // get theme ids
      themes = filter_object(nodeca.config.theme_schemas, function (theme) {
      ...
      Async.forEach(init_env.apps, function (app, next_app) {
        // for each theme
        Async.forEach(_.keys(themes), function (theme_id, next_theme) {
          process_assets({
            handler : move,
    ...
    function apply_patches(patches, next) {
      ...
      // parallel forEach
      Async.forEach(_.keys(patches), function (orig_file, next_file) {
    ...
    function bundle_files(files, next) {
      ...
      Async.forEachSeries(_.keys(files), function (orig_file, next_file) {
    ...
    function localize_views(files, next) {
      ...
      views_files = _.filter(files, function (f) {
      ...
      Async.forEachSeries(views_files, function (file, next_file) {
        ...
        Async.forEachSeries(nodeca.config.locales.enabled, function (lang, next_lang) {
    ...
    function compile_jade_assets(files, next) {
      var views_files, filter_re, dbg = ('development' === nodeca.runtime.env);
      ...
      Async.forEachSeries(views_files, function (file, next_file) {
        Async.waterfall([
          ...
          function (data, callback) {
            ...
            // FIXME: decouple
            init_env.registerView(file.match(filter_re)[1].replace(/\//g, '.'), Jade.compile(data, {
              compileDebug: dbg,
              client: false
            }));
    ...
    function compile_styl_assets(files, next) {
      ...
      re = /[.]styl$/i;
      styl_files = _.filter(files, function (f) {
        return re.test(f);
      });
    ...
    function minify_assets(files, next) {
      ...
      re = /[.]js$/i;
      js_files = _.filter(files, function (f) {
        return re.test(f);
      });
