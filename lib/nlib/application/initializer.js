...
module.exports.initialize = function initialize(callback) {
  var stage = function (hook, func) {
    //  *** WARNING ********************************************************
    // `Async.apply()` calls function with null context, that cause problems
    //  with calling instance methods. So using our own wrapper.
    return function (next) { nodeca.hooks.init.run(hook, func, next); };
  };

  Async.series([
    stage('init-start',   null),
