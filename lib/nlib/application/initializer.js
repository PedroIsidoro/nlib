...
// TODO:  Respect server module path and proxy filters.before/after calls
//        to allow "relative" hook names (list, ::admin.users.list and @.list)
...
function init_config(next) {
  ...
  Async.series([
    ...
    function (next) {
      // FIXME: Because of config merges - arrays are converted to objects
...
function init_router(next) {
  ...
  _.each(config.map || {}, function (routes, api_path) {
    var prefix = find_mount_point(api_path, default_mount, config.mount || {});
      ...
      pointer.addRoute(pattern, {
        ...
        handler: function () {
          return {
            ...
            method_func: HashTree.get(nodeca.server, api_path)
          };
        }
      });
    });
