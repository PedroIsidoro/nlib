...
function init_router(next) {
  ...
  // FIXME: Fail init on "invalid" api_paths
  // TODO: respect mount points

  //
  // fill in routes
  _.each(config.map || {}, function (routes, api_path) {
    var prefix = default_mount, func;
  ...
  _.each(config.redirects || {}, function (options, old_pattern) {
    ...
    route = new Pointer.Route(options.to.shift(), {
      params: options.params,
      handler: function () {}
    });
