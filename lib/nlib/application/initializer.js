...
function init_logger(mainCfg, next) {
...
function init_databases(mainCfg, next) {
...
function load_sub_apps(mainCfg, next) {
  var config = {};

  Async.forEachSeries(Underscore.keys(mainCfg.applications), function (app_name, next_app) {
    var app;

    if (!mainCfg.applications[app_name]) {
      next_app();
      return;
    }
    ...
    load_app_configs(app, function (err, cfg) {
      init_env.apps.push(app);
      Common.mergeConfigs(config, cfg);
      next_app(err);
    });
  }, function (err) {
    Common.mergeConfigs(config, mainCfg);
...
module.exports.initialize = function initialize(main, callback) {
  ...
  Async.waterfall([
    // preparation stages without hooks
