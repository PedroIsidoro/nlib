...
function load_translations(next) {
  ...
  Async.forEachSeries(nodeca.runtime.apps, function (app, next_app) {
    ...
    FsTools.walk(locales_path, function (file, stats, next_file) {
      ...
      if (!nodeca.runtime.locales[locale]) {
        nodeca.runtime.locales[locale] = data || {};
      } else {
        Underscore.extend(nodeca.runtime.locales[locale], data);
      }
...
function init_translations(next) {
  ...
  Underscore.each(enabledLocales, function (locale) {
    Underscore.each(nodeca.runtime.locales[locale], function (data, scope) {
      nodeca.runtime.translator.addPhrase(locale, scope, data);
    });
  });

  // fill in aliases via fallbacks
  Underscore.each(enabledLocales, function (fallback, locale) {
    nodeca.runtime.translator.setFallback(locale, fallback);
  });
