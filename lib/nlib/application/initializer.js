...
function apply_main_config(next) {
  ...
  _.each(nodeca.config, function (val, key) {
    if ('^' === key[0]) {
      delete nodeca.config[key];

      if (nodeca.runtime.env === key.substr(1)) {
        Common.mergeConfigs(nodeca.config, val);
      }
    }
  });
...
function mark_enabled_themes(next) {
  ...
  _.each(nodeca.config.theme_schemas, function (opts, id) {
