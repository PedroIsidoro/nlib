...
function build_bundles(next) {
  StaticBundler.bundle(nodeca, function (err, bundle_dir) {
    // TODO: Read some of the files (views, translations) into API tree here
    FsTools.walk(bundle_dir, function (file, stats, next_file) {
      // Fill in Static lulz with files and data
      Async.waterfall([
        Async.apply(Fs.readFile, file),
        function (buffer, callback) {
          var rel_path = file.replace(bundle_dir, '');
...
function init_assets_server(next) {
  // TODO: init_assets_server? it's initialized once buindler build bundles
  next();
}
