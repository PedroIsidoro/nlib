...
function init_translations(next) {
  ...
  enabledLocales  = (cfg['enabled']) ? cfg['enabled'] : Underscore.keys(nodeca.config.i18n || {});
  ...
  Underscore.each(enabledLocales, function (locale) {
    Underscore.each(nodeca.config.i18n[locale], function (data, scope) {
...
module.exports.initialize = function initialize(callback) {
  var stage = function () {
    ...
    func  = (1 === steps.length) ? steps.shift()
          : Async.apply(Async.series, steps);
