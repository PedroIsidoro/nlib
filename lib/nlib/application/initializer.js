...
// inline regexp into real RegExp objects. Used by `init_router()`.
...
//
// - change nodeca.filter.{before|after} to understand relative api_path
// - execute given block (second arguments)
// - restore original nodeca.filter.{before|after}
...
// process router config files and fill router with:
// - routes
// - redirectors
// - direct invocators
...
/** internal
 *  Application.Initializer.initialize(stages, callback) -> Void
 *  Application.Initializer.initialize(callback) -> Void
 **/
module.exports.initialize = function initialize(stages, callback) {
  var tasks = [], steps = [
    ['init-start',     null],
    ['models-tree',    load_models],
    ['server-tree',    Async.apply(load_api_subtree, 'server')],
    ['shared-tree',    Async.apply(load_api_subtree, 'shared')],
    ['client-tree',    Async.apply(load_api_subtree, 'client')],
  ...
  steps.forEach(function (step) {
    var hook = step[0], func = step[1];

    if (null === stages || 0 <= stages.indexOf(hook)) {
      tasks.push(function (next) {
        nodeca.hooks.init.run(hook, func, next);
      });
    }
  });
