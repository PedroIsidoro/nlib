...
//  dump(obj) -> String
//
//  Dumps given `obj` into JSON object containing only one key `dump` with
//  "dumped" (serialized) value of `obj`. Use eval in order to unpack it later:
//
//  var str = dump(obj);
//  var obj = eval(JSON.parse(str).dump);
function dump(obj) {
...
/**
 *  StaticBundler.bundle(nodeca, callback(err, bundle_dir)) -> Void
 ...
 **/
module.exports.bundle = function(nodeca, callback) {
  ...
  bundle_tmp = tmpname('source');  // Temporary directory for assets, langs, etc.
  ...
  Async.waterfall([
    ...
    function build_language_files(next) {
      Async.forEach(nodeca.config.languages.enabled, function (lang, next_lang) {
        var data = nodeca.runtime.translator.getCompiledData(lang, '', {deep: true});
        Async.forEach(Underscore.keys(data), function (ns, next_ns) {
          var tmp_file, dst_file;
          
          tmp_file = tmpname('i18n-' + lang + ns);
          dst_file = Path.join(bundle_tmp, 'system', ns, 'i18n', lang + '.json');

          Async.series([
            Async.apply(Fs.writeFile, tmp_file, dump(data)),
            Async.apply(FsTools.copy, tmp_file, dst_file),
            Async.apply(FsTools.remove, tmp_file)
          ], next_ns);
        }, next_lang);
      }, next);
    },
    ...
    function collect_views_and_assets(next) {
      ...
      Async.forEach(nodeca.runtime.apps, function (app, next_app) {
        process_assets({
    ...
    function localize_views(files, next) {
      ...
      filter_re = new RegExp('^' + Path.join(bundle_tmp, 'views'));
      ...
      Async.forEachSeries(views_files, function (file, next_file) {
        ...
        Async.forEachSeries(nodeca.runtime.languages.enabled, function (lang, next_lang) {
          // TODO: localize_views is stubbed and needs real implementation
          FsTools.copy(file, base + '.' + lang + '.jade');
        }, function (err) {
          if (err) {
            next_file(err);
            return;
          ...
          // remove original file after loclaized versions were created
          FsTools.remove(file, next_file);
        });
      }, function (err) {
        next(err, files);
      });
    ...
    function compile_jade_assets(files, next) {
      // TODO: compile_jade_assets not implemented
      next(null, files);
    },
    //
    //
    ...
    function minify_assets(files, next) {
      ...
      // TODO: minify_assets not implemented
      next();
    }
  ], function (err) {
