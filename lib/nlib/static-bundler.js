...
// returns handler suitable for `end`, `close`, `error` events of stream
function fin_stream(stream, callback) {
  return function (err) {
    stream.destroy();
    callback(err);
  };
...
function concat_files(dst_file) {
  ...
  src_file_iterator = function (file, next) {
    Path.exists(file, function (exists) {
      ...
      // assign error/end listeners
      set_stream_listener(src, ['error', 'end'], callback);
    });
  };

  // assign error listener
  set_stream_listener(tmp, ['error'], callback);
