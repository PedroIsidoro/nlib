...
// merge all src_files into dst_file.
// will create/truncate dst_file first.
function merge_files(dst_file, src_files, callback) {
  var dst = Fs.createWriteStream(dst);

  Async.forEachSeries(src_files, function (file, next_file) {
...
module.exports.bundle = function(nodeca, apps, destination, callback) {
  ...
  Async.waterfall([
    ...
    function apply_patches(patches, next) {
      ...
      Async.forEach(Underscore.keys(patches), function (orig_file, next_file) {
        Path.exists(orig_file, function (exists) {
          ...
          tasks.push(function (next_task) {
            var tmp_file, src_files;
           
            tmp_file = orig_file + '.tmp';
            src_files = [].concat(patches[orig_file].before.sorted)
                          .concat([orig_file])
                          .concat(patches[orig_file].after.sorted);

            Async.waterfall([
              Async.apply(merge_files, tmp_file, src_files),
              Async.apply(Async.forEach, src_files, Fs.unlink),
              Async.apply(Fs.rename, tmp_file, orig_file)
            ], next_task);
          });
    ...
    function bundle_files(files, next) {
      Async.forEach(Underscore.keys(files), function (orig_file, next_file) {
        Path.exists(orig_file, function (exists) {
          var tmp_file, src_files;

          tmp_file = orig_file + '.tmp';
          src_files = (exists ? [orig_file] : []).concat(files[orig_file]);
