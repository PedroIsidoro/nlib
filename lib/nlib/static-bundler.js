...
module.exports.bundle = function(nodeca, apps, destination, callback) {
  ...
  Async.waterfall([
    ...
    //
    //
    // 3.1.A) collect unified patches
    ////////////////////////////////////////////////////////////////////////////
    function collect_unified_patches(next) {
      var patches, re;

      re = /^(.+?)[.](\d+)[.]_[a-z0-9]{8}_[.]patch$/i;
      patches = {};

      FsTools.walk(source_tmp, re, function (filename, stats, next_file) {
        var match, realpath, priority;
        
        match = filename.replace(source_tmp, '').match(re);
        realpath = match[1];
        priority = match[2];

        // for each found path, create new stack if not found
        if (!patches[realpath]) {
          patches[realpath] = new Types.SortedSet();
        }

        patches[realpath].add(priority, filename);
        next_file();
      }, function (err) {
        next(err, patches);            
      });
    ...
    //
    //
    // 3.1.B) apply unified patches
    ////////////////////////////////////////////////////////////////////////////
    function apply_unified_patches(patches, next) {
      // parallel forEach
      Async.forEach(Underscore.keys(patches), function (orig_file, next_file) {
        // waterfall forEach
        Async.forEachSeries(patches[orig_file].sorted, function (patch_file, next_patch) {
