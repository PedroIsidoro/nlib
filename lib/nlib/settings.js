...
var KNOWN_TYPES = _.keys(DEFAULTS);
...
var Settings = module.exports = function Settings() {
  ...
  verify_options = function verify_options(options) {
    ...
    var unknown_key = _.keys(options).reject(function (k) {
      return _.include(VALID_KEYS, k);
    }).shift();

    // at least one key is invalid
    if (unknown_key) {
      throw new Error("Unknown setting key=" + unknown_key);
    }

    // invalid type
    if (!_.include(KNOWN_TYPES, options.type)) {
      throw new Error("Unknown setting type=" + options.type);
    }
  };
  ...
  standard_definition = function standard_definition(key, options) {
    ...
    if (has_base_definition) {
      throw new Error("Duplicate base definition of setting key=" + key);
    }
    ...
    _(key2stores[key]).chain().select(function (store) {
      return !!data[store].definitions[key].extend;
    }).each(function (store) {
   ...
   **/
  this.get = function get(key, env, callback) {
    if (undefined === key2stores[key]) {
      callback(new Error("Unknown key=" + key + " specified"));
      return;
    }
    ...
    Async.map(key2stores[key], function (name, next) {
      self.store(name).get(key, env, next);
    }, function (err, data) {
      var result, args;
   ...
   **/
  this.store = function (name, store) {
   ...
   **/
  this.definition = function definition(store, key, options) {
...
// expose Store
Settings.Store = require('./settings/store');
