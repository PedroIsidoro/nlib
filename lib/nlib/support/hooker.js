...
Hooker.prototype.run = function run(bucket) {
  var hooks, before, after, args, block, callback, async_len;

  hooks     = get_hooks(this, bucket);
  args      = Array.prototype.slice.call(arguments, 1);
  callback  = args.pop(); // last arg - callback
  block     = [args.pop()]; // one before last - block
  async_len = args.length + 1;
  before    = hooks.before.sorted;
  after     = hooks.after.sorted;

  Async.forEachSeries([].concat(before, block, after), function (fn, next) {
    if (fn.length === async_len) {
      fn.apply(this, args.concat([next]));
    } else {
      try { next(fn()); }
      // this should NEVER happen, as you must avoid throwing errors, but
      // return them instead. unfortunately shit happens.
      catch (err) { next(err); }
    }
  }, callback);
};
