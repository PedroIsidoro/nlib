...
//  find_theme_files(id, dir, callback(err, files)) -> Void
//  - id (String): Theme id, e.g. `default-desktop`
//  - dir (String): Path to directory with `views` and `assets`
//  - callback (Function): Fired with `err` and list of `files`
//
//  Returns list of theme files. Used to clone 
function find_theme_files(id, dir, callback) {
  var files = [], re = new RegExp('^' + dir + '/(?:views|assets)/theme-' + id + '/');

  FsTools.walk(dir, re, function (path, stats, next) {
...
module.exports.bundle = function(nodeca, apps, destination, callback) {
  ...
  Async.waterfall([
    ...
    function make_unique_filenames(next) {
      ...
      FsTools.walk(assets_tmp, /[.](patch|before|after)$/i, function (file, stats, next) {
        // mixin md5 hash of original path
        FsTools.move(file, Builder.unique_name(file), next);
      }, next);
    },
    //
    ...
    function process_inherited_themes_clone_base(next) {
      ...
      worker = function (base_id, id, callback) {
        var base_re = build_theme_regexp(base_id, destination);

        // FsTools.walk walks ONLY files, so no need to check stats
        FsTools.walk(assets_tmp, base_re, function (src, stats, next) {
          var match = re.exec(src), dst;
