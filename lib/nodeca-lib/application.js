...
var init_database = function init_database(callback) {
  ...
  var uri = get_mongoose_uri(this.config.database);

  this.mongoose = new (require('mongoose')).Mongoose();
...
var init_models = function init_models() {
  ...
  $$.each(this.schemas, function (name, schema) {
    ...
    models[name] = mongoose.model(nice_name, schema, schema.get('collection'));
  });
...
/**
 *  Application#readConfig(filename[, env]) -> Object
 *
 *  Reads and parses `config/<filename>.yml` file of application. If `env` is
 ...
 **/
Application.prototype.readConfig = function readConfig(filename, env) {
  var file = path.join(this.dirname, 'config', filename + '.yml'), data;

  if (!path.exists(file)) {
    throw Error('Config file not found. file=' + file);
  }

  data = $$.merge({}, $$.readYamlSync(file));
