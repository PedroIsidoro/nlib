...
var Application = module.exports = function Application(dirname, bootstrapper) {
  ...
  this.embedInto = function embedInto(mainApp) {
    ...
    // resolve local promises with main app
    main.getLogger(logger_ready.resolve);
    main.getMongoose(mongoose_ready.resolve);
    main.getDispatcher(dispatcher_ready.resolve);
    main.getAllApplications(apps_ready.resolve);
    main.getAllSchemas(schemas_ready.resolve);
    main.getAllModels(models_ready.resolve);
    main.getAllViews(views_ready.resolve);
    main.getAllControllers(controllers_ready.resolve);
    main.getAllRouters(routers_ready.resolve);
    main.getStaticBuilder(static_builder_ready.resolve);
    main.getStatcLulz(static_lulz_ready.resolve);
