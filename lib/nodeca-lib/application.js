...
var init_database = function init_database() {
  ...
  this.connection = require('mongoose').createConnection(uri);
};
...
var load_controllers = function load_controllers() {
  ...
  if (fs.statSync(dirname).isDirectory()) {
    fs.readdirSync(dirname).forEach(function (file) {
      ...
      controllers[name] = new controller(app);
    });
  }
...
var create_cache_getter = function create_cache_getter(cache) {
  return (function get(name, strict) {
    if (cache.hasOwnProperty(name)) {
      return cache[name];
    }

    if (!strict && this.super !== this) {
      return this.super.getSchema(name);
    }

    this.log.error("Schema '" + name + "' not found");
    return null;
  }).bind(this);
};
...
var Application = module.exports = function Application(dirname, bootstrapper) {
  ...
  **/
  ...
  this.getApp = function getApp(name) {
    ...
    this.log.error("Application '" + name + "' not embedded");
    return null;
  };


  this.getSchema = create_cache_getter.call(this, schemas);


  this.getModel = create_cache_getter.call(this, models);


  this.getController = create_cache_getter.call(this, controllers);
};
