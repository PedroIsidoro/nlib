...
var Application = module.exports = function Application(dirname, bootstrapper) {
  ...
  this.init = function init(appConfig, callback) {
    ...
    self.getLogger(function (err, log) {
      if (err) {
        callback(err);
        return;
      }
      ...
      self.getAppName(function (err, name) {
        ...
        self.getConfig(function (err, config) {
          ...
          self.bootstrap(function (err) {
            var bootstrapped_joint = new Promise.Joint();
            ...
            // load and bootstrap sub-apps
            $$.each(config.modules, function(name, subappConfig) {
              var subapp = null,
                  subapp_promise = new Promise();

              log.debug('Loading subapp=' + name);
              bootstrapped_joint.include(subapp_promise);

              try {
                subapp = require('nodeca-' + name);
              } catch (err) {
                bootstrapped_joint.reject(err);
                return;
              }
              ...
              subapp.getConfig(function (err, config) {
                ...
                subapp.bootstrap(function (err) {
                  ...
                  subapp_promise.resolve();
                });
              });
            });

            bootstrapped_joint.wait().done(function (err) {
              if (err) {
                callback(err);
                return;
              }

              hooks.exec('bootstrapped', function (err) {
                var schemas_joint = new Promise.joint();
                ...
                $$.each(apps, function (name, subapp) {
                  var schemas_promise = new Promise();

                  log.debug('Loading schemas for app=' + name);
                  schemas_joint.include(schemas_promise);

                  subapp.getSchemas(function (err, schemas) {
                    if (err) {
                      schemas_joint.reject(err);
                      return;
                    }

                    schemas_promise.resolve();
                  });
                });

                schemas_joint.wait().done(function (err) {
                  if (err) {
                    callback(err);
                    return;
                  }

                  hooks.exec('schemas-loaded', function (err) {
                    var models_joint = new Promise.joint();

                    // init models
                    $$.each(apps, function (name, subapp) {
                      var models_promise = new Promise();

                      log.debug('Loading models for app=' + name);
                      models_joint.include(models_promise);

                      subapp.getModels(function (err, models) {
                        if (err) {
                          models_joint.reject(err);
                          return;
                        }

                        models_promise.resolve();
                      });
                    });

                    models_joint.wait().done(function (err) {
                      callback(Error("Not implemented fully yet"));
                    });
                  });
                });
              });
