...
var path          = require('path'),
    ...
    StaticManager = require('./static_manager'),
    $$            = require('./utilities');
  ...
  if (path.existsSync(dirname)) {
    fs.readdirSync(dirname).forEach(function (file) {
      ...
      if (suffix_re.test(file)) {
        ...
        debug('  > loaded: ' + name + ' (' + instance.name + ')');

        controllers[self.name + '/' + name] = instance;
        self.dispatcher.addController(instance);
      }
    });
  }
...
var Application = module.exports = function Application(dirname, bootstrapper) {
  ...
  **/
  this.init = function init(config) {
    ...
    var self = this,
        log = this.log.info('Initializing ' + this.name || '<unknown>');
    ...
    // build static manager
    log.info('*** Collecting static files...');
    var static_manager = new StaticManager();
    $$.each(apps, function (name, app) {
      var static_path = path.join(app.dirname, 'public');
      if (path.existsSync(static_path)) {
        log.debug('Adding app=' + name + ' static files from ' + static_path);
        static_manager.add(static_path);
      }
    });
    log.info('*** Compiling static files...');
    static_manager.compile();
    this.staticManager = static_manager;
...
/**
 *  Application#getPackageInfo(callback) -> Void
 *
 ...
 **/
Application.prototype.getPackageInfo = function getPackageInfo(callback) {
