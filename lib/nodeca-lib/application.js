...
var Application = module.exports = function Application(dirname, bootstrapper) {
  ...
  this.init = function init(appConfig, callback) {
    ...
    prepare_mainapp = function (callback) {
    ...
    prepare_subapps = function (callback) {
      var prepare_all = new Promise.Joint(function (err) {
        if (err) {
          callback(err);
          return;
        }

        hooks.exec('bootstrapped', callback);
      });

      log.info('Loading sub applications');

      $$.each(apps, function (name, subappConfig) {
        var subapp = null,
            subapp_ready = new Promise();

        log.debug('Loading subapp=' + name);
        prepare_all.include(subapp_ready);

        try {
          subapp = require('nodeca-' + name);
        } catch (err) {
          prepare_all.reject(err);
          return;
        }
