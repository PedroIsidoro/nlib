...
/**
 *  Application.getPackageInfo(dirname) -> Object
 *
 *  Returns hash representing data of `package.json` under `dirname`, or empty
 *  `object` on any error.
 *
 *  **NOTICE** File should be UTF-8 encoded.
 **/
Application.getPackageInfo = function getPackageInfo(dirname) {
  try {
    var file = path.join(dirname, 'package.json');
