...
var Application = module.exports = function Application(dirname) {
  ...
  // Hash of loaded apps hash {name: app}
  var apps = {};
  ...
  **/
  this.init = function init(config) {
    ...
    init_database(this, config); // initialize database
    apps[this.name] = this; // register ourself in the applications hash

    // load apps
    $$.each(this.__config__.modules || {}, function (name, config) {
      var app = apps[name] = require(name).embedInto(this);
      $$.deepMerge(app.__config__, config);
    });
    ...
    // TODO: handle after-bootstrap

    // load schemas
    $$.each(apps, function (name, app) {
      schemas = $$.deepMerge({}, loadSchemas(app), schemas);
    });

    // TODO: handle after-schema loaded

    // load models
    $$.each(apps, function (name, app) {
      models = $$.deepMerge({}, loadModels(app), models);
    });

    // TODO: handle after-models loaded

    // load controllers
    $$.each(apps, function (name, app) {
      controllers = $$.deepMerge({}, loadControllers(app), controllers);
    });
