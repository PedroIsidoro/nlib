...
var Joint = module.exports = function Joint(callback) {
  ...
  var self = this, // self-reference
      ...
      try_resolve; // resolves joint if necessary


  // helper for [[Promise.Joint#include]]
  try_resolve = function try_resolve() {
  ...
  /** chainable
   *  Promise.Joint#wait([p1[, p2[, pN]]]) -> Promise.Joint
   *
   ...
   *
   *  You may pass `Promises` to be included, e.g.:
   *
   *      join.include(cfg_ready, db_ready).wait();
   *
   *  equals to:
   *
   *      join.wait(cfg_ready, db_ready);
   *
   *
   *  ##### Example
   *
