...
var init_config = function init_config(next) {
  var context = this;

  context.main.getConfig(function (err, config) {
    if (err) {
      next(err);
      return;
    }

    context.config = $$.merge(config, context);
    next();
  });
};
...
var bootstrap_main = function bootstrap_main(next) {
  var context = this;

  context.main.bootstrap(function (err) {
    if (err) {
      next(err);
      return;
    }
...
var load_subapps = function load_subapps(next) {
  ...
  $$.each(context.config.modules, function (module, config) {
    var app,
    ...
    app.getConfig(function (err, appConfig) {
      if (err) {
        finished.reject(err);
        return;
      }
