...
var create_stack = function create_stack(context) {
  ...
  return {
    queue: function queue(handler, hookName) {
      queue.push(handler.bind(context));

      if (hookName) {
        queue.push(function handle_hook(next) {
          context.hooks.exec(hookName, next);
        });
      }
...
// });
module.exports = function init(appConfig, callback) {
  var context = this;
  ...
  self.getLogger(function (err, logger) {
    ...
    self.bootstrap(function (err) {
      ...
      create_stack(context)
        .queue(load_subapps, 'subapps-loaded')
        .queue(load_schemas, 'schemas-loaded')
        .queue(load_models, 'models-loaded')
        .queue(load_controllers, 'controllers-loaded')
        .queue(load_routes, 'routes-loaded')
        .queue(load_static_storage, 'init-complete')
