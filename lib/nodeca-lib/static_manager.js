...
var StaticManager = module.exports = function StaticManager() {
  ...
  this.compile = function compile() {
    ...
    var styl_re = new RegExp(/\.styl$/);

    // process stylus files
    vfs.each(styl_re, require('./static_manager/stylus'));

    // remove original styl files
    vfs.each(styl_re, function(path, data) {
      vfs.add(path.replace(styl_re, '.css'), data.buffer.css);
      vfs.drop(path);
    });
